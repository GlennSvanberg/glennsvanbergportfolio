...
import { fetchBlogPostBySlug } from '~/lib/convexServer'
import {
  NOINDEX_ROBOTS,
  buildBlogPostPageMeta,
  ...
  head: ({ loaderData, params }) => {
    ...
    if (!post) {
      return buildPageMeta({
        ...
        description: 'Det begärda blogginlägget finns inte på glennsvanberg.se.',
        path: `/blog/${params.slug}`,
        robots: NOINDEX_ROBOTS,
      })
    }
    return buildBlogPostPageMeta(post)
  },
  component: BlogPost,
})
...
function BlogPost() {
  ...
  const livePost = useQuery(api.posts.getBySlug, { slug })
  const post: Doc<'posts'> | null =
    livePost !== undefined ? livePost : loaderPost
  ...
  if (post === null) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center bg-[var(--background)] text-foreground">
        <h1 className="text-4xl font-black uppercase tracking-tighter mb-4">
          Inlägget hittades inte
        </h1>
        <Link
          to="/blog"
          className="text-primary font-mono uppercase tracking-widest hover:text-emerald-400 transition-colors"
        >
        ...
        <header className="mb-16">
          <div className="flex flex-wrap items-center gap-3 mb-8">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="text-primary bg-primary/5 font-mono text-xs tracking-widest uppercase px-3 py-1 rounded-full border border-primary/20"
              >
                {tag}
              </span>
            ))}
            <time
              ...
              className="text-muted/60 font-mono text-xs tracking-widest uppercase px-3 py-1 rounded-full border border-primary/10"
            >
